...
```{r}
data <- read.csv("C:/Users/ADMIN/Downloads/bodyPerformance (1).csv", stringsAsFactors = FALSE)
...
data_filter_scale <- data_filter %>%
  mutate_at(vars(age, height_cm, weight_kg, body_fat_percent, diastolic, systolic, grip_force, sit_and_bend_forward_cm, sit_ups_counts, broad_jump_cm),
            scale)
...
logistic_model <- glm(class_binary ~ age + grip_force + sit_ups_counts + broad_jump_cm + body_fat_percent + systolic + diastolic + bmi,
             data = train_data,
             family = binomial)
